...
#' @param use.tidyr logical, whether to use \code{tidyr::separate_rows}
#'
...
uwSplit <- function(data, use.tidyr=TRUE) {
  ...
  if (use.tidyr) {
    ...
    # 构造一个循环，将原始的data数据按行写入到新的数据框中，写入行数与主承销商数
    # 量相同，如果有5个主承销商则会重复5行，这是通过cum序列和行下标实现的，参考
    # a1和a2的设定
